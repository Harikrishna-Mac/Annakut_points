...
export default function InspectorActivityPage() {
  ...
  const [inspectorTransactions, setInspectorTransactions] = useState<InspectorTransaction[]>([]);
  ...
  const fetchInspectorTransactions = async (inspectorEmail: string) => {
    ...
    try {
      let url = `/api/inspector-activity?inspectorEmail=${encodeURIComponent(inspectorEmail)}`;
      const params = new URLSearchParams();
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-red-50">
      ...
      <div className="bg-white/60 backdrop-blur-sm border-b">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex space-x-8 overflow-x-auto">
            ...
            <Link href="/inspector-activity" className="py-4 px-2 border-b-2 border-orange-500 text-orange-600 font-medium whitespace-nowrap">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-slate-800 mb-4">Inspector Activity Dashboard</h2>
          <p className="text-slate-600 max-w-2xl mx-auto">Monitor all inspector activities and track point transactions across the system.</p>
        </div>
        ...
        <div className="mb-8">
          <h3 className="text-xl font-bold text-slate-800 mb-4">Inspector Summary</h3>
          {isLoading ? (
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-8 text-center border border-white/20">
              ...
              <p className="text-slate-600">No inspector activity found for the selected period.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {activities.map((activity, index) => (
                <div
                  key={index}
                  className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-shadow cursor-pointer"
                  onClick={() => fetchInspectorTransactions(activity.user_email)}
                >
                  ...
                  <h4 className="font-bold text-slate-800 mb-1 truncate">{activity.user_name}</h4>
                  <p className="text-sm text-slate-500 mb-4 truncate">{activity.user_email}</p>

                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-slate-600">Sevaks Affected:</span>
                      <span className="font-semibold text-slate-800">{activity.unique_sevaks}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-slate-600">Total Transactions:</span>
                      <span className="font-semibold text-slate-800">{activity.total_transactions}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-green-600">Points Added:</span>
                      <span className="font-semibold text-green-600">+{activity.total_added}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-red-600">Points Deducted:</span>
                      <span className="font-semibold text-red-600">-{activity.total_deducted}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-blue-600">Attendance Points:</span>
                      <span className="font-semibold text-blue-600">+{activity.attendance_points}</span>
                    </div>
                  </div>

                  <button className="mt-4 w-full px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-lg text-sm font-medium transition-colors">
                    View Details →
                  </button>
                </div>
              ))}
                  ...
                  <table className="w-full">
                    <thead className="bg-slate-50 border-b">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-center text-sm font-semibold text-slate-700">Points Change</th>
                        <th className="px-6 py-4 text-center text-sm font-semibold text-slate-700">Before → After</th>
                      </tr>
                    </thead>
                    <tbody>
                      {inspectorTransactions.map((transaction) => (
                        <tr key={transaction.id} className="border-b hover:bg-slate-50/50 transition-colors">
                          <td className="px-6 py-4">
                            <div className="text-sm text-slate-600">{formatDateTime(transaction.device_timestamp)}</div>
                          </td>
                          <td className="px-6 py-4">
                            <div>
                              <div className="font-semibold text-slate-800">{transaction.sevak_name}</div>
                              <div className="text-sm text-slate-500">{transaction.sevak_id}</div>
                            </div>
                          </td>
